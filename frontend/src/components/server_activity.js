...
const ServerActivity = () => {
    ...
    useEffect(() => {
        ...
        const fetchFlaskStatus = async () => {
            try {
                const response = await axios.get('http://localhost:3000/api/v1/health/flask');
                setFlaskStatus(response.data.message === 'Flask server is healthy' ? 'running' : 'error');
            } catch (error) {
        ...
        const fetchExpressStatus = async () => {
            try {
                const response = await axios.get('http://localhost:3000/api/v1/health/express');
                setExpressStatus(response.data.message === 'Express server is healthy' ? 'running' : 'error');
            } catch (error) {
        ...
        const fetchMongoDBStatus = async () => {
            try {
                const response = await axios.get('http://localhost:3000/api/v1/health/mongodb');
                setMongoDBStatus(response.data.message === 'MongoDB connection is open' ? 'open' : 'error');
            } catch (error) {
        ...
        <div className="container mt-4">
            ...
            <div className="row">
                <div className="col-md-4">
                    <div className={`card ${flaskStatus === 'running' ? 'bg-success' : 'bg-danger'} mb-4`}>
                ...
                <div className="col-md-4">
                    <div className={`card ${expressStatus === 'running' ? 'bg-success' : 'bg-danger'} mb-4`}>
                        <div className="card-body">
                ...
                <div className="col-md-4">
                    <div className={`card ${mongodbStatus === 'open' ? 'bg-success' : 'bg-danger'} mb-4`}>
                        <div className="card-body">
